...
xInputs = np.linspace(0, 1, inputs.shape[-1]) * 0.3
yInputs = (inputs[indices, :] * 0.3).reshape(len(indices), -1)
...
for i in range(len(indices)):
    ...
    axes[0].plot(
        xInputs + proyeccion[indices[i], 0],
        yInputs[i] + proyeccion[indices[i], 1],
        color="black",
        linewidth=2,
    )
