...
def generar_quivers(img: np.ndarray, quiver: np.ndarray, index: tuple):
    ...
    xWin, yWin = np.meshgrid(
        np.arange(0, 75 * 2, 4), np.arange(0, 75 * 2, 4)
    )  # SALE 38x38
    plt.imshow(img[index], cmap="Greys", vmin=0, vmax=1)
    plt.quiver(
        xWin, yWin, quiver[index, :, :, 0], -quiver[index, :, :, 1], color="yellow"
    )
    plt.xticks([])
    plt.yticks([])
...
def generar_grafica_ls(
    data: np.ndarray, index: tuple, vmin: float = None, vmax: float = None
):
...
def plotCollection(
    ax: plt.Axes, xs: np.ndarray, ys: np.ndarray = None, *args, **kwargs
):
...
# Para el scatter
i = 1
for centro, vecinos in zip(
    (centroP0, centroP1, centroP2), (indicesVecinos0, indicesVecinos1, indicesVecinos2)
):
    axes["A"].scatter(centro[:, 0], centro[:, 1], color=colores[i], marker="x")
    axes["A"].scatter(
        proyeccion[vecinos, 0],
        proyeccion[vecinos, 1],
        color=colores[i],
        marker="x",
        s=25,
        linewidths=3,
