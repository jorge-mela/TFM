...
axes[0].scatter(
    proyeccion[:, 0], proyeccion[:, 1], alpha=0.3, color=get_cmap("Set1")(0.222)
)
axes[1].set_ylim(-1, 1)
...
for i, punto in enumerate(puntos):
    ...
    axes[0].scatter(
        proyeccion[indicesVecinos, 0],
        proyeccion[indicesVecinos, 1],
        color=colores[i],
        marker="x",
        s=30, # Para el tamaño de las cruces,
        linewidths=3 # Para el grosor de las cruces
    )
    plotCollection(
        ax=axes[1],
        xs=datosLS[indicesVecinos].T,
        color=colores[i],
        label=str(punto),
        alpha=0.7,
    )
