...
MODELO_ENCODER = "encoder_CAE2D_03_25_K2.keras"
MODELO_DECODER = "decoder_CAE2D_03_25_K2.keras"
...
def generar_plots(data: np.ndarray, indice:int, vmin:int, vmax:int):
    ...
    plt.colorbar(ScalarMappable(norm=norm, cmap="viridis"), ticks=(0, 1))
    plt.imshow(data[indice], cmap="viridis", norm=norm)
    plt.xticks(())
...
for i, indx in enumerate(indices):
    ...
    generar_plots(datos_visu, indx, 0, 1)
...
for indx in range(len(indices)):
    ...
    generar_plots(datosDecodificados_visu, indx, 0, 1)
