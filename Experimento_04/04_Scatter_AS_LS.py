...
NOMBRE_ARCHIVO = "CAE2D_03_proyecciones_25_K2.hdf5"
PATH_DATOS = "./Experimento_04/proyecciones/"
PATH_FIGURES = "./Experimento_04/figures/"

N_CHANNELS = 1
SIZE_FIG = 8
ZOOM_IMGS = SIZE_FIG / 14

# Para colorear las imágenes
colorMap = cm.Colormap("viridis", 256)
normLS = cm.Normalize(vmin=-1, vmax=1)
normAS = cm.Normalize(vmin=0, vmax=1)
...
inputs_s = MinMaxScaler((0, 1)).fit_transform(inputs.reshape((-1, N_CHANNELS)))
...
fig, axes = plt.subplots(2, 1, figsize=(2*SIZE_FIG, SIZE_FIG), sharey=True, sharex=True)
fig.tight_layout()
...
for i in range(len(indices)):
    ...
    im = OffsetImage(inputs[indices[i]], zoom=ZOOM_IMGS, cmap=colorMap, norm=normAS)
...
for i in indices:
    im = OffsetImage(inputs[i], zoom=ZOOM_IMGS, cmap=colorMap, norm=normAS)
    img = im.get_children()[-1]
    img.set_cmap("viridis")
